document.addEventListener('DOMContentLoaded', () => {
    ...
    const timelineWrap = document.querySelector('.experience-timeline');
    ...
    function renderSlides() {
      ...
      const filtered = data.filter(item => activeType === 'all' || item.type === activeType);
      ...
      filtered.forEach((item, idx) => {
        ...
        milestone.innerText = item.company.split(',')[0];
        milestone.addEventListener('click', () => showSlide(idx));
        timelineWrap.appendChild(milestone);
      });
    ...
    function showSlide(n) {
      ...
      milestones[currentIndex].classList.add('active');
    }
